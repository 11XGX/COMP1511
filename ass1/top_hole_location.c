// Author: Nathan Ellis
// Date: 30 April 2017
// Top Hole Location - This function uses a four-pass flood-fill algorithm to isolate holes and then checks for the value of the highest row position of the hole.
...
int get_top_hole_location(int height, int width, int pixels[height][width]) {
	...
	// Check for earliest row of the hole
	for (int row = height - 1; row > -1; row--) {
		for (int column = width - 1; column > -1; column--) {
			if (pixels[row][column] == 0) {
				return row;
